...
class ChainService {
  ...
  /**
   * Parse a token amount string to BN (in planck units)
   */
  public parseBalance(amount: string, decimals: number = NETWORK_CONFIG.tokenDecimals): BN {
    // Remove commas and spaces
    const cleaned = amount.replace(/,/g, '').replace(/\s/g, '').replace(NETWORK_CONFIG.tokenSymbol, '');
